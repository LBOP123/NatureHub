  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      ...
      <el-form-item label="识别结果" prop="recognitionResult">
        <el-input v-model="queryParams.recognitionResult" placeholder="请输入识别结果" clearable size="small" @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="上传时间">
        <el-date-picker v-model="dateRange" size="small" type="daterange" range-separator="-" start-placeholder="开始日期" end-placeholder="结束日期" />
      </el-form-item>
      <el-form-item>
      ...
      <el-row :gutter="20" v-if="form">
        <el-col :span="12">
          <div class="image-preview">
            <el-image style="width: 100%; border-radius: 8px;" :src="form.imageUrl" :preview-src-list="[form.imageUrl]" fit="contain" />
          </div>
        </el-col>
        <el-col :span="12">
          <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="置信度">
              <el-progress :percentage="Number((parseFloat(form.confidence) * 100).toFixed(0))" :color="getConfidenceColor(form.confidence)"></el-progress>
            </el-descriptions-item>
            <el-descriptions-item label="状态">
              <dict-tag :options="statusOptions" :value="form.statusLabel"/>
            </el-descriptions-item>
            <el-descriptions-item label="识别类型">
              <dict-tag :options="typeOptions" :value="form.recognitionTypeLabel"/>
            </el-descriptions-item>
...
export default {
  ...
  created() {
    this.getDicts('bio_recognition_status').then(response => {
      this.statusOptions = response.data
    })
    this.getDicts('bio_recognition_type').then(response => {
...
.image-preview { padding: 10px; background: #f5f7fa; border-radius: 8px; }
.content-box {
