...
public class UserCommunityController extends BaseController
{
                                      ...
                                      @PathVariable("sourceId") Long sourceId)
    {
        try {
            ...
            if (isObservation) {
                ObservationRecord rec = observationRecordService.selectObservationRecordByRecordId(sourceId);
                return rec != null ? success(rec) : error("原始观察记录不存在");
            } else if (isIdentification) {
                SpeciesIdentification rec = speciesIdentificationService.selectSpeciesIdentificationByIdentificationId(sourceId);
                return rec != null ? success(rec) : error("原始鉴定记录不存在");
            } else if (isSurvey) {
                FieldSurvey rec = fieldSurveyService.selectFieldSurveyBySurveyId(sourceId);
