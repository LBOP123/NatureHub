...
public class BioRecognitionController {
    ...
    /**
     * 保存识别记录（成功）
     * 【改造点】status 由 "success" 改为 1；recognitionType 由字符串改为数字
     */
    private void saveRecognitionRecord(BioRecognitionRequest request, BioRecognitionResponse response, HttpServletRequest httpRequest) {
        try {
            ...
            // 【改造点】识别类型：字符串 -> 数字  plant=1, animal=2, other=3
            String typeStr = response.getType();
            if ("plant".equals(typeStr)) {
                record.setRecognitionType(1);
            } else if ("animal".equals(typeStr)) {
                record.setRecognitionType(2);
            } else {
                record.setRecognitionType(3);
            }
